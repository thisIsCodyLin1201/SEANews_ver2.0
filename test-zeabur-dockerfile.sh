...
echo "[2/6] 清理舊容器和映像..."
docker rm -f seanews-zeabur-test 2>/dev/null || true
docker rmi seanews:zeabur-test 2>/dev/null || true
echo ""

echo "[3/6] 構建 Zeabur 優化的 Docker 映像..."
echo "注意：這可能需要 3-5 分鐘"
if ! docker build -t seanews:zeabur-test .; then
    echo ""
...
echo "測試結果"
echo "===================================="
docker logs seanews-zeabur-test 2>&1 | grep -E "Application startup complete|ERROR|ModuleNotFoundError"
echo ""
