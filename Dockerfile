...
# Zeabur 會自動設置 WEB_PORT 環境變量，無需手動指定
# 健康檢查使用默認端口 8080（Zeabur 會在運行時覆蓋）
HEALTHCHECK --interval=30s --timeout=10s --start-period=40s --retries=3 \
    CMD curl -f http://localhost:${WEB_PORT:-8080}/api/health || exit 1

# 啟動命令 - 使用 ${WEB_PORT:-8080} 確保有默認值
# Zeabur 會自動注入 WEB_PORT 環境變量，如果沒有則使用 8080
CMD ["sh", "-c", "python -m uvicorn agno_api:app --host 0.0.0.0 --port ${WEB_PORT:-8080} --log-level info"]
