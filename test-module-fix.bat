@echo off
REM Docker 模組導入修復測試腳本
REM 用於在網路條件允許時重新構建和測試
...
echo [1/5] 清理舊容器和映像...
docker rm -f seanews-module-test 2>nul
docker rmi seanews:module-fix 2>nul
echo.

echo [2/5] 構建新映像（應用模組導入修復）...
echo 注意：這可能需要 5-10 分鐘，取決於網路速度
docker build -t seanews:module-fix .
if errorlevel 1 (
    echo.
...
echo [3/5] 啟動測試容器...
docker run -d --name seanews-module-test -p 8787:8787 seanews:module-fix
if errorlevel 1 (
    echo [錯誤] 容器啟動失敗！
    pause
    exit /b 1
...
echo [5/5] 檢查容器日誌...
docker logs seanews-module-test 2>&1 | findstr /C:"Application startup complete" /C:"ModuleNotFoundError" /C:"ERROR"
echo.
